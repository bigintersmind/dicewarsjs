...
- **AI System** (src/ai/): Contains different AI strategies:
  ...
  - ai_conqueror / ai_blitz / ai_survivor: the player-facing self-play **personas** (docs/ml-bot/PERSONAS.md, [D-27]) — the in-game + arena/tournament roster, each a makeBC({policy}) over its own weights. **Conqueror** = the ppo-long weights (balanced, the strongest net the game ships); **Blitz** = a short-horizon fine-tune that closes games fast; **Survivor** = a placement-reward fine-tune that outlasts the field (it beats ppo-long head-to-head, Δ +8.4 pp). Tagged `persona` in builtInBots.js, which keeps them OUT of the canonical ppo:gate reference field (challengers, not baselines)

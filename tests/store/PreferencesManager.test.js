...
describe('PreferencesManager', () => {
  ...
  describe('coachHints', () => {
    it('defaults to on and round-trips a valid value through localStorage', () => {
      const pm = createPreferencesManager();
      expect(pm.get('coachHints')).toBe('on');
      expect(pm.set('coachHints', 'off')).toBe(true);
      expect(pm.get('coachHints')).toBe('off');
      pm.destroy();

      // A second manager reads the persisted value rather than the default.
      const reloaded = createPreferencesManager();
      expect(reloaded.get('coachHints')).toBe('off');
      reloaded.destroy();
    });
    ...
    it('rejects values off the on/off pair', () => {
      ...
      for (const bad of ['yes', true, 1, null]) {
        expect(pm.set('coachHints', bad)).toBe(false);
      }
      expect(pm.get('coachHints')).toBe('on');
      warnSpy.mockRestore();
      pm.destroy();
    ...
    it('ignores an invalid persisted value', () => {
      localStorage.setItem('dicewars_prefs', JSON.stringify({ coachHints: 'maybe' }));
      const pm = createPreferencesManager();
      expect(pm.get('coachHints')).toBe('on');
      pm.destroy();
    });

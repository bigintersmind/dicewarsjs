 ...
 * @property {number[] | null} candidateAreas - Territories the coaching layer
 *   is currently offering the human player: every territory that could start an
 *   attack while awaiting `selectFrom`, or the reachable enemies of
 *   `selectedFrom` while awaiting `selectTo`. Null whenever no coaching applies
 *   — the `coachHints` preference is off, nobody is playing

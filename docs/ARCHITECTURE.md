...
| `src/renderer/`   | PixiJS rendering: hex grid drawing, dice sprites, battle animations. Four overlay layers sit above the territories; the board-hint layer is the lowest of them, beneath the keyboard focus ring, the dice, and the `from`/`to` selection, so the committed selection stays dominant. Reads from GameStore, never mutates game state.                                                                            |
| `src/ui/`         | Preact components: title screen, game HUD, arena screen, tournament screen, replay viewer, leaderboard, `RulesModal` (the how-to-play card, store-driven like `QuitConfirm` and mounted outside the screen switch so it opens over any screen), and `BoardFocus` (the board's DOM focus target — one hidden button per live territory, whose names carry the board's state on the human's turn (#211 item 10)). |

...
- **Renderer** (src/renderer/): PixiJS rendering layer — `GameRenderer` (top-level), `HexGridRenderer` (territory drawing with border tracing, plus four overlay layers; `setCandidateHighlights(ids, kind)` paints a whole board-hint set into one Graphics, and that layer is the LOWEST overlay — beneath the keyboard focus ring, the dice, and the `from`/`to` selection — so the committed selection stays dominant; `clearSelectionHighlights()` is the mid-game clear — selection + hints, focus ring untouched, the ring being the keyboard cursor and visible exactly when `store.focusedAreaId` is set — while `clearHighlights()` is the full wipe, for the seams that also null `focusedAreaId`), `DiceRenderer` (isometric stacked dice), `BattleAnimation` (physics-based dice rolling).

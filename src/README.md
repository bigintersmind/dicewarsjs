...
An ES module application: Vite build, PixiJS v8 rendering, Preact UI, all running on a pure game engine in `src/engine/`.
...
Arena bots use a different interface: the sandboxed `state → { from, to } | null` function
that tournaments run. See [`docs/BOT_GUIDE.md`](../docs/BOT_GUIDE.md) to write one.

...
- **Renderer** (src/renderer/): PixiJS rendering layer — `GameRenderer` (top-level), `HexGridRenderer` (territory drawing with border tracing, plus four overlay layers stacked board hints → `from` → `to` → keyboard focus; `setCandidateHighlights(ids, kind)` paints a whole hint set into one Graphics, each ring a bright core over a dark halo so it survives the bright player palette), `DiceRenderer` (isometric stacked dice), `BattleAnimation` (physics-based dice rolling).

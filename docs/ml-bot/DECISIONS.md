...
## D-33 — Wave-2 Predator revival CLOSED under the current wire: neither pilot clears the §10.8 gate · Accepted (2026-07-08) · resolves the [PERSONAS.md](./PERSONAS.md) §10.2 Predator slot

**Context.** With the pre-Wave-2 kill-gates all ratified ([D-32](#d-32--wave-1-persona-acceptance-ship-blitz-v3-on-strength-over-a-dual-control-signature-flag-survivor-v3-killed--keep-v2-predators-comparator--v2-survivor--accepted-2026-07-06--resolves-the-personasmd-102-wave-1-ship-decisions) follow-ups (a)/(b): scavenge tripwires 0.31/5.64/0.91, kills bar +0.26, winPct floor 35, clock-hack 0.05/0.31/0.18), the §10.2 Predator pilots launched on shodan 2026-07-08 — two 1M placement + elim-bounty arms warm-started from the encoding-v3 base (`ppo-v3-scratch/ppo.pt`), identical but for the bounty coefficient: **`ppo-v3-pred-b15`** (ELIM_BOUNTY 0.15) and **`ppo-v3-pred-b25`** (0.25), both SHAPING_CLIP 1.0, γ0.999. Graded at 0.5M (tripwire probe) and 1M (full battery) on the mini in the §10.3 calibration field; strength (gate + curve) on shodan. Numbers: RESULTS.md 2026-07-08 (§10.2). ([PR #134] — the #85 `elimsByLearner` tracker fix — was deliberately LEFT OUT per Ivan; it cannot fire in the 8-FFA config (MapGenerator guarantees every seat starts alive), so the realized reward stream is byte-identical with or without it.)

**The grades.**

- **`ppo-v3-pred-b15` (bounty 0.15) — strong and clean, but NOT a Predator.** BEAT the Lookahead floor decisively (paired Δwin% **+24.7 [20.2, 29.2]**, 648 games); the eval-stream curve is three BEAT points tied within CI (35.0 → 33.5 → 31.5% win, no [D-29] tail-regression kill). Every tripwire panel clear — and it is the _anti_-turtle (reserve Δ−8.0, games 26 turns faster, `zeroAttackTurnFrac` Δ−0.007 vs base). It separates from Survivor on turnsToWin + avgPlacement. **But its kill-edge over v2 Survivor is Δ+0.16 [0.00, 0.31] — short of the +0.26 confirmatory bar** (and of the realized 0.244 relative MDE). It does not out-scavenge the comparator by the margin that earns a distinct "predator" slot.
- **`ppo-v3-pred-b25` (bounty 0.25) — out-kills Survivor, but falls into the basin.** It clears the kills bar (Δ+0.39 [0.17, 0.61] vs Survivor; separates on kills) — the higher bounty _does_ buy the scavenging. But it is **KILLED by the [D-30](#d-30--batch-2b-dense-persona-re-pilot-corrected-reward-mechanics-the-1γ-residual-γ-transformed-expansionist-placement-backbone-predator-one-pre-registered-flag-only-wave--accepted-2026-07-01--resolves-batch2_repilot_findingsmd-6) turtle-basin tripwire on `zeroAttackTurnFrac`**: Δ+0.178 [0.110, …] vs base at 0.5M — **> 2× the +0.05 threshold** (the `kill-on-2×` tier) — and still Δ+0.072 [0.046, 0.098] at 1M (point estimate over threshold, CI excludes 0 → the `kill-at-1M-on-any` tier). Both probes fire independently. _(Nuance for the record: b25 is not a *classic* turtle — it is faster than base and better-placed; the single basin axis it trips is the passive-turn proxy, i.e. the 0.25 bounty taught it to sit and wait for kill windows on a fraction of turns. The pre-registered axis fires regardless — see the consequence.)_

**The complementary failure is the finding.** Across the {0.15, 0.25} bounty sweep on the current reward wire (placement + elim-bounty + shaping-clip 1.0, γ0.999), the kills come _only_ with basin-tripping passivity (b25), or clean aggressive play comes _without_ enough kill-margin over Survivor (b15). Neither corner yields a bot that both out-kills Survivor AND stays out of the basin — the two constraints pull apart. One piece of evidence must survive for any future revisit: **the dose-response sign flipped from v2 to v3** — v2 kills fell monotonically as the bounty rose (the §10.2 arm-table note), v3 kills RISE with it (+0.16 → +0.39) with a clean scavenge panel (genuine hunting, not vulturing). `eliminatesDefender` fixed the _direction_ of representability; what stayed binding is the _price_ — by the time the bounty clears the kills bar, the policy has bought a passive wait-for-kill-windows posture. That is the signature §10.3's hypothesis 2 (turn-boundary credit dilution) predicts, and it is what makes the frame-level fix an evidence-backed live hypothesis rather than a hail-mary.

**Decision (Ivan's standing ruling, applied): Predator CLOSED under the current wire.** §10.8's Predator kill-gate fires: b15 fails the confirmatory kills bar; b25 clears it but is independently basin-killed — so **no pilot clears the full gate**. Per the pre-committed consequence: **revisitable only with a frame-level kill-attribution fix; no coefficient re-sweeps under this wire.** The natural revisit is exactly what b25's nuance exposes — `zeroAttackTurnFrac` is a coarse proxy for the degenerate behavior; a frame-level kill-attribution fix — a [D-28]-pattern header/timing change that lands kill credit on the killing-blow decision instead of diluted at the turn boundary (§10.3 hypothesis 2), paired with a passivity metric that distinguishes "waiting for a kill" from "stalling" — is the ticket to a future Predator, not a bounty re-tune. (That fix is **not** the parked #85 / [PR #134] guard: that one seeds the tracker against a latent start-eliminated-seat mis-credit and, per the hold-out rationale above, changes nothing on this wire.)

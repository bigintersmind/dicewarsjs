...
export class HexGridRenderer {
  ...
  constructor(parent) {
    ...
    /*
     * Coaching affordance layer (the "Coach" prototype): every territory that
     * can act right now, drawn into ONE Graphics. Added before the selection
     * overlays so it stays beneath them — the committed from/to selection must
     * stay the dominant mark on the board — while drawMap's addChildAt(gfx, 0)
     * keeps it above the territories themselves.
     */
    /** @type {Graphics} Soft highlight overlay for attack candidates */
    ...
    /** @type {number[]} Territories currently marked by the coaching layer */
    this._candidateIds = [];
    /** @type {'attacker' | 'target'} Treatment the coaching layer is painting */
  ...
  setTheme(theme) {
    ...
    /*
     * Themes predating the coaching layer (and hand-built test doubles) may not
     * carry the candidate colors; fall back to the dark palette's rather than
     * writing `undefined` into a Graphics stroke.
     */
  ...
  drawMap(state) {
    ...
    // Borders are being retraced from scratch: anything the coaching layer drew
    // against the previous map's outlines is now stale geometry.
  ...
  /**
   * Softly highlight a set of territories as "you can act on these right now"
   * (the "Coach" prototype's affordance layer). Replaces whatever the layer
   * held before — one call paints the whole set.
   *
  ...
  /** Clear the coaching affordance layer. */
  ...
  /**
   * Repaint the candidate layer from `_candidateIds` — also the hook that keeps
   * it correct across a theme change or a redraw.
   */
  _redrawCandidates() {
     ...
     * rim first and the bright core on top, so neither disappears on the bright
     * half of the player palette (lime, cyan, yellow).
     */
  ...
  /** Clear all selection highlights (including keyboard focus and coaching candidates). */
  clearHighlights() {

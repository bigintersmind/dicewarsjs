...
- **AI System** (src/ai/): Contains different AI strategies:
  ...
  - Luck handicap (#179): the orthogonal SECOND difficulty axis, independent of the lineup presets — the title screen's "Your luck" row writes `config.luck` (0/1/2, the `LUCK_LEVELS` ladder in `src/utils/config.js`) into the store, and `luckToHandicap(luck, humanPlayerIndex)` turns it into the engine's `config.handicap = { playerId, level } | null` (the human seat rolls `level` extra dice and drops the `level` lowest, attacking and defending). Null everywhere else by construction: spectator/attract games have no human seat, and arena/tournament/leaderboard never pass it. Replay v2 whitelists it, so a handicapped game replays exactly; rejectMap must rebuild it or the setting vanishes on NEW MAP.

...
- **GameController** (src/controller/GameController.js): Orchestrates the full game loop — title → mapPreview → playing → gameOver. Handles human input (two-phase click: select from, select to; a primary-button click on water — `handleTerritoryClick(0)`, forwarded by `src/controller/canvasPointer.js`, the tested body of the canvas `pointerdown` listener — cancels a half-made attack through `cancelSelection()`, which the keyboard's Escape delegates to; #211 follow-ups 15/16/22), AI turns with step-by-step animation, and turn advancement. Also the single owner of the board hints (#196 carve-out): `refreshCandidateHighlights()` derives `store.candidateAreas` from the engine's own `getValidMoves` and paints it, so board and rules can't drift; gated on the `boardHints` preference and null for a spectator or an AI's turn. Drives the engine for the real game; its sibling `TitleAttractMode.js` steps a private, decorative AI-vs-AI engine game behind the menu screens (`ATTRACT_SCREENS`: title + arena/tournament/leaderboard, whose chrome floats over the scrimmed live board — heuristic bots only, so no persona weight chunks load on the landing page).
...
- **Renderer** (src/renderer/): PixiJS rendering layer — `GameRenderer` (top-level), `HexGridRenderer` (territory drawing with border tracing, plus four overlay layers; `setCandidateHighlights(ids, kind)` paints a whole board-hint set into one Graphics, and that layer is the LOWEST overlay — beneath the keyboard focus ring, the dice, and the `from`/`to` selection — so the committed selection stays dominant; `clearSelectionHighlights()` is the mid-game clear — selection + hints, focus ring untouched, the ring being the keyboard cursor and visible exactly when `store.focusedAreaId` is set — while `clearHighlights()` is the full wipe, only for a seam that leaves the playing screen and nulls `focusedAreaId` in the same breath (quit to title)), `DiceRenderer` (isometric stacked dice), `BattleAnimation` (physics-based dice rolling).

- **UI** (src/ui/): Preact components — `App` (screen router), `TitleScreen`, `MapPreview`, `GameHUD`, `GameOverlay`, `GameOverScreen`, `RulesModal` (the "How to play" card: mounted globally outside the screen switch, driven by the `rulesOpen` store flag, reachable from the title screen / in-game bar / game-over screen, and its capture-phase Escape closes it before QuitConfirm sees the key), `ScreenReaderAnnouncer` (the one `role="status"` live region, also mounted outside the screen switch since #211 item 9 so it is one stable node for the whole session — `useAnnouncer` speaks only on `playing`/`gameOver` and resets to `''` off them, because a same-string `setState` is a no-op and would leave the next game's identical line silent). Uses `useGameStore` hook for reactive updates. Route-change focus convention (#189): every Preact screen reached by activating a control that unmounts with the old screen moves focus to its primary on mount — TitleScreen START, MapPreview PLAY, GameOverScreen HOME, TopNav's `aria-current` tab, with `MenuScreen focusTitleOnMount` as an opt-in headline fallback (the playing screen is the exception: it deliberately places no focus, so the walk starts at the top of the screen; since #211 the board itself is real DOM — `BoardFocus` renders a `role="application"` group of `.sr-only` territory buttons between RULES and END TURN, so document order is the tab order and the browser walks it, with the human's own territories tabbable on their turn and everything else arrow-reachable only, and each button's name carrying, on the human's turn, the board's state from the engine's own `getValidMoves` (`can attack` / `selected` / `no enemy neighbor` / `valid target` / `not a valid target`) — the same rule that gates `handleTerritoryClick` and feeds the hints, so label, click and hint cannot disagree (#211 item 10, #204); `store.focusedAreaId` mirrors DOM focus and the controller keeps just the arrows, E and Escape — and stands all three down while the quit confirm, the "How to play" card or the settings dropdown is open, the store's `quitConfirmOpen` / `rulesOpen` / `settingsOpen` flags; #211 item 8). Each claims focus exactly once (`focus({ preventScroll: true })` at mount — GameOverScreen defers its one claim until the "How to play" card closes when the game ended behind it) and never again, so it never yanks focus mid-interaction.

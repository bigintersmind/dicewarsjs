...
def test_len_and_item_shapes(tmp_path):
    ...
    item = ds[0]
    assert item["nodes"].shape == (6, 13)  # v3: 13 node features
    assert item["players"].shape == (2, 7)
    assert item["board"].shape == (7,)
    assert item["edge_feat"].shape[1] == 10  # v3: 10 edge features

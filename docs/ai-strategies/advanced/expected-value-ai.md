...
- **Chokepoint awareness on defense.** Leaving the attacking cell at 1 die is
  charged not just as exposure but, if that cell is one of _your_ articulation
  points, as the income you would lose if it were then recaptured.
...
- **Anti-runaway.** If any player holds more than ~40% of all dice on the board,
  ...
  slightly penalized — the same gang-up dynamic described in
...
interval (Student's t). Read the intervals, not just the ranks:
...
1. You want a single, tunable objective rather than a brittle priority list of
   rules.
2. Reinforcement/connectivity effects matter — i.e. on most real maps.
